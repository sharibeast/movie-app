...
export const Recommendation = () => {
  ...
  return (
    ...
    <div className=' flex py-4 text-white bg-primary-bg px-2 md:px-16  md:py-10'>
      <div>
        {/* search bar Input */}
        <SearchInputBar />
        {/* mobile nav --end */}
        <div className='grid gap-4'>
          ...
          <div>
            <div className='flex justify-between mb-8'>
              ...
              <button className='text-gray-500 text-base md:text-xl capitalize px-8'>see all</button>
            </div>
...
export const DesktopSideBar = () => {
  return (
    <div className='bg-primary-bg pl-4 w-3/5 sm:w-[400px] h-screen  grid gap-4 -mx-4'>
      ...
      <div className='flex items-center justify-between'>
        <div className='text-xl opacity-60'>categories</div>
        <button className='text-gray-01 text-sm'>uncheck all</button>
      </div>

      <div className='p-4 bg-[#121212] rounded-xl  grid gap-2'>
        {checkBoxData.map((chk, idx) => <Checkbox key={idx} text={chk.text} />)}
      </div>

      <div className='flex items-center justify-between'>
        <div className='text-xl opacity-60'>Services</div>
        <button className='text-gray-01 text-sm'>uncheck all</button>
      </div>


      <div className='p-4 bg-[#121212] rounded-xl mt-8 grid gap-2'>
        {checkBoxData.map((chk, idx) => <Checkbox key={idx} text={chk.text} />)}
      </div>
    </div>
  )
...
export const SearchInputBar = () => {
  ...
  return (
    <>
      ...
      {sidebar ? (
        <div className='bg-primary-bg p-6 w-3/5 sm:w-[400px] lg:hidden h-screen z-10 grid gap-4 absolute top-0 right-0 -mx-4'>
          <div className='flex items-center justify-end'>
            <button onClick={() => setSidebar(false)} className='mr-auto'><CloseIcon /></button>
            <div className='p-1  bg-gray-01 rounded-full opacity-50'>
              <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M17.25 12V10C17.25 7.1005 14.8995 4.75 12 4.75C9.10051 4.75 6.75 7.10051 6.75 10V12L4.75 16.25H19.25L17.25 12Z"></path>
                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 16.75C9 16.75 9 19.25 12 19.25C15 19.25 15 16.75 15 16.75"></path>
              </svg>
            </div>
            <img src='/src/assets/images/pr1.jpeg' className='h-6 w-6 rounded-full' />
          </div>
          <div className='flex items-center justify-between'>
            <div className='text-xl opacity-60'>categories</div>
            <button className='text-gray-01 text-sm'>uncheck all</button>
...
export const Checkbox = ({ text }: { text: string }) => {
  return (
    <div className='flex items-center opacity-60 justify-between border-b border-gray-01 py-2'>
      {text}
      <input className='accent-[#3DD2CC] h-4 w-4' type={"checkbox"} />
    </div>
  )
}
